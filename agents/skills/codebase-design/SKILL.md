...
**Deep module** = small interface + high leverage:
...
├─────────────────────┤
│                     │
│  Deep Implementation│  ← Many behaviours hidden
│                     │
└─────────────────────┘
```
...
```
┌─────────────────────────────────┐
│       Large Interface           │  ← Caller learns most complexity
├─────────────────────────────────┤
│  Thin Implementation            │  ← Little leverage
└─────────────────────────────────┘
```
...
1. **Accept infrastructure at module composition.** Do not construct it inside an operation or make every caller pass it to every call. Keep the dependency seam internal unless choosing the adapter is part of the caller's job.
...
2. **Make outcomes observable.** Command interfaces may encapsulate side effects. Return a result, emit a domain event, or expose the outcome through an injected adapter so callers and tests can observe behaviour without reaching past the interface.
